...
public interface ILogWidget : IWidget {
    ...
    public static bool ParseAlertStatus(ButtonComponent button) {
        return button.CustomId switch {
            "alerts-toggle:True" => true,
            "alerts-toggle:False" => false,
    ...
    public static bool ParseAlertStatus(IEnumerable<ActionRowComponent> actionRows) {
        return actionRows.GetComponentById("alerts-toggle:True") != null;
    }
    public static bool ParseAlertStatus(IMessage message) {
        var components = message.Components as IEnumerable<ActionRowComponent>;
        return ParseAlertStatus(components);
    }
    /// <summary>
